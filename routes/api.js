...
router.get('/addapikey', (req, res, next) => {
    ...
    if (apikey != 'Xcode') return res.json(loghandler.invalidKey)
...
router.get('/cewe/vietnam', async (req, res, next) => {
        var apikeyInput = req.query.apikey
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'Xcode')  return res.json(loghandler.invalidKey)
...
router.get('/nsfw/femdom', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if (apikeyInput != 'Xxode')  return res.json(loghandler.invalidKey)
...
router.get('/nsfw/foot', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if (apikeyInput != 'Xode')  return res.json(loghandler.invalidKey)
...
router.get('/muslim/doaharian', async (req, res, next) => {
	...
	if(apikeyInput != 'Xcode') return res.json(loghandler.invalidKey)
       ...
       fetch(encodeURI(`https://raw.githubusercontent.com/greggygf/doa-harian-muslim-server/master/db.json`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
